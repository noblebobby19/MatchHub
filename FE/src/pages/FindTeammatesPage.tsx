...
import {
  Users, MapPin, Clock, Calendar, ArrowLeft, Trophy,
  Plus, Loader2, History, Trash2, Eye, ChevronRight, UserCheck, Phone
} from "lucide-react";
...
export function FindTeammatesPage() {
  ...
  // Fetch lich su khi chuyen sang tab
  useEffect(() => {
  ...
  const fetchMyPosts = async () => {
    ...
    try {
      const data = await apiService.getMyPosts('teammate');
      setMyPosts(data);
    } catch (error) {
  ...
  const fetchJoinedTeams = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch joined teams:', error);
      toast.error('Không thể tải danh sách đội đã tham gia');
    } finally {
  ...
  const getStatusBadge = (post: Post) => {
    if (post.neededPlayers === 0 || post.status === 'closed') {
      return <Badge className="bg-gray-100 text-gray-600">Đã đủ người</Badge>;
    }
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl">
                {activeTab === 'history' ? 'Lịch sử tìm đội'
                  : activeTab === 'joined' ? 'Đội đã tham gia'
                    : 'Tìm đồng đội'}
              </h1>
              <p className="text-lg opacity-90 mt-1">
                {activeTab === 'history'
                  ? 'Các bài đăng tìm đồng đội của bạn'
                  : activeTab === 'joined'
                    ? 'Những đội bạn đã được chấp nhận tham gia'
            ...
            <Badge className="bg-white/20 hover:bg-white/30">
              {activeTab === 'browse'
                ...
                : activeTab === 'history'
                  ...
                  : `${joinedTeams.length} đội đã tham gia`}
            </Badge>
          </div>
        </div>
      </div>
                ...
                <>
                  {posts.map((post) => (
                    <PostCard
                      ...
                      onView={() => navigate(`/chi-tiet-bai-dang/${post._id}`)}
                    />
                  ))}
                  {posts.length === 0 && (
                    <div className="text-center py-20">
                      ...
                      <Button onClick={handleCreatePost}>Đăng tin ngay</Button>
                    </div>
                  )}
                </>
              )}
                ...
                <>
                  {myPostsLoading ? (
                    ...
                    <div className="text-center py-20">
                      ...
                      <p className="text-muted-foreground mb-6">
                        Hãy tạo bài đăng để tìm đồng đội!
                      </p>
                      <Button
                        onClick={handleCreatePost}
                    ...
                    <div className="space-y-4">
                      ...
                      {myPosts.map((post) => (
                        <Card key={post._id} className="hover:shadow-md transition-shadow border-l-4 border-l-blue-500">
                          <CardContent className="p-5">
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex items-start gap-4 flex-1 min-w-0">
                                <div className="h-12 w-12 bg-gradient-to-br from-blue-100 to-blue-200 rounded-xl flex items-center justify-center text-xl font-bold text-blue-700 flex-shrink-0">
                                  ...
                                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                                    <div className="flex flex-col gap-2">
                                      ...
                                      <span className="flex items-center gap-1">
                                        ...
                                        {post.currentPlayers} người
                                        {post.neededPlayers > 0 && (
                                          <span className="text-blue-600 font-medium ml-1">
                                            (Thiếu {post.neededPlayers})
                                          </span>
                                        )}
                                      </span>
                                    </div>
                ...
                <>
                  {joinedLoading ? (
                    ...
                    <div className="text-center py-20">
                      ...
                      <h3 className="mb-2">Bạn chưa đăng nhập</h3>
                      <p className="text-muted-foreground mb-6">Đăng nhập để xem đội đã tham gia.</p>
                      <Button onClick={() => navigate('/dang-nhap')}>Đăng nhập</Button>
                    </div>
                  ) : joinedTeams.length === 0 ? (
                    <div className="text-center py-20">
                      <UserCheck className="h-16 w-16 text-muted-foreground mx-auto mb-4" />
                      <h3 className="mb-2">Bạn chưa tham gia đội nào</h3>
                      <p className="text-muted-foreground mb-6">
                        Vào xem các bài đăng và gửi yêu cầu tham gia đội!
                      </p>
                    ...
                    <div className="space-y-4">
                      <p className="text-sm text-muted-foreground mb-2">
                        Bạn đã tham gia <strong>{joinedTeams.length}</strong> đội
                      </p>
                              ...
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex items-start gap-4 flex-1 min-w-0">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap items-center gap-2 mb-1">
                                      ...
                                      <Badge className="bg-green-100 text-green-700 text-xs">Đã tham gia</Badge>
                                    </div>
                                ...
                                <div className="flex-shrink-0">
                                  <Button
                                    ...
                                    variant="outline"
                                    className="text-blue-600 border-blue-200 hover:bg-blue-50"
                                    onClick={() => navigate(`/chi-tiet-bai-dang/${joinedPost._id}`)}
                                  >
                                    <Eye className="h-3.5 w-3.5 mr-1" />
                                    Xem chi tiết
                                  </Button>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <h3 className="text-lg font-bold">{post.teamName || 'Đội bóng'}</h3>
                  {isOwner && (
                    <Badge className="bg-blue-100 text-blue-700 text-xs">Bài của bạn</Badge>
                  )}
                </div>
                <p className="text-sm text-muted-foreground mb-2">
